...
/**
 *   	\file       verifactuincidencia_list.php
 *		\ingroup    verifactu
 *		\brief      List page for verifactuincidencia
 */
...
require_once __DIR__.'/class/verifactuincidencia.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('verifactu', 'verifactuincidencia', 'write');
	$permissiontodelete = $user->hasRight('verifactu', 'verifactuincidencia', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("verifactu")) {
	accessforbidden('Module verifactu not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'VerifactuIncidencia';
	$objectlabel = 'VerifactuIncidencia';
	$uploaddir = $conf->verifactu->dir_output;
...
$title = $langs->trans("VerifactuIncidencias");
//$help_url = "EN:Module_VerifactuIncidencia|FR:Module_VerifactuIncidencia_FR|ES:Módulo_VerifactuIncidencia";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/verifactu/verifactuincidencia_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-verifactu page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/verifactu/verifactuincidencia_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendVerifactuIncidenciaRef";
$modelmail = "verifactuincidencia";
$objecttmp = new VerifactuIncidencia($db);
