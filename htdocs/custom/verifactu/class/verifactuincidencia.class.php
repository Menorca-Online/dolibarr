...
/**
 * \file        class/verifactuincidencia.class.php
 * \ingroup     verifactu
 * \brief       This file is a CRUD class file for VerifactuIncidencia (Create/Read/Update/Delete)
 */
...
/**
 * Class for VerifactuIncidencia
 */
class VerifactuIncidencia extends CommonObject
{
	/**
	 * @var string 		ID of module.
	 */
	public $module = 'verifactu';

	/**
	 * @var string 		ID to identify managed object.
	 */
	public $element = 'verifactuincidencia';

	/**
	 * @var string 		Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'verifactu_verifactuincidencia';

	/**
	 * @var string 		If permission must be checkec with hasRight('verifactu', 'read') and not hasright('mymodyle', 'verifactuincidencia', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'verifactu';

	/**
	 * @var string 		String with name of icon for verifactuincidencia. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'verifactuincidencia@verifactu' if picto is file 'img/object_verifactuincidencia.png'.
	 */
	...
	public $fields = array(
		...
		"persona_nombre" => array("type" => "varchar(255)", "label" => "Personanombre", "enabled" => "1", 'position' => 15, 'notnull' => 1, "visible" => "-1",),
		"persona_apellido_1" => array("type" => "varchar(255)", "label" => "Personaapellido1", "enabled" => "1", 'position' => 20, 'notnull' => 1, "visible" => "-1",),
		"persona_apellido_2" => array("type" => "varchar(255)", "label" => "Personaapellido2", "enabled" => "1", 'position' => 25, 'notnull' => 0, "visible" => "-1",),
		"persona_nif" => array("type" => "varchar(20)", "label" => "Personanif", "enabled" => "1", 'position' => 30, 'notnull' => 1, "visible" => "-1",),
		"persona_email" => array("type" => "varchar(255)", "label" => "Personaemail", "enabled" => "1", 'position' => 35, 'notnull' => 1, "visible" => "-1",),
		"persona_telefono" => array("type" => "varchar(50)", "label" => "Personatelefono", "enabled" => "1", 'position' => 40, 'notnull' => 0, "visible" => "-1",),
		"incidencia" => array("type" => "text", "label" => "Incidencia", "enabled" => "1", 'position' => 45, 'notnull' => 1, "visible" => "-1",),
	);
	public $rowid;
	public $persona_nombre;
	public $persona_apellido_1;
	public $persona_apellido_2;
	public $persona_nif;
	...
	//  */
	// public $table_element_line = 'verifactu_verifactuincidencialine';
	...
	//  */
	// public $fk_element = 'fk_verifactuincidencia';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'VerifactuIncidencialine';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'VerifactuIncidencia', 'fk_element'=>'fk_verifactuincidencia'));
	...
	//  */
	// protected $childtablesoncascade = array('verifactu_verifactuincidenciadet');

	// /**
	//  * @var VerifactuIncidenciaLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('verifactu', 'verifactuincidencia', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('verifactu', 'verifactuincidencia', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('verifactu', 'verifactuincidencia_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'verifactuincidencia/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'verifactuincidencia/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'verifactuincidencia/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'verifactuincidencia/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->verifactu->dir_output.'/verifactuincidencia/'.$oldref;
				$dirdest = $conf->verifactu->dir_output.'/verifactuincidencia/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->verifactu->dir_output.'/verifactuincidencia/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('verifactu','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('verifactu','verifactu_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'VERIFACTU_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'VERIFACTU_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'VERIFACTU_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowVerifactuIncidencia")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("VerifactuIncidencia").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/verifactu/verifactuincidencia_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowVerifactuIncidencia");
	...
	public function getLinesArray()
	{
		...
		$objectline = new VerifactuIncidenciaLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_verifactuincidencia:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('VERIFACTU_MYOBJECT_ADDON')) {
			$conf->global->VERIFACTU_MYOBJECT_ADDON = 'mod_verifactuincidencia_standard';
		}

		if (getDolGlobalString('VERIFACTU_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('VERIFACTU_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('VERIFACTU_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/verifactu/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefVerifactuIncidencia $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_verifactuincidencia';
		...
		$modelpath = "core/modules/verifactu/doc/";
...
/**
 * Class VerifactuIncidenciaLine. You can also remove this and generate a CRUD class for lines objects.
 */
class VerifactuIncidenciaLine extends CommonObjectLine
{
	// To complete with content of an object VerifactuIncidenciaLine
	// We should have a field rowid, fk_verifactuincidencia and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'verifactuincidencia'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_verifactuincidencia'
